...
int evaluate() {
	...
	for (int i = 0; i < len; i++) {
		for (int j = i + 1; j < len; j++) {
			//a = (int)(solution[i] - solution[j]) / (double)(i - j);
			if ((int)(solution[i] - solution[j]) / (double)(i - j) == 1 || ((int)(solution[i] - solution[j])) / (double)(i - j) == -1) 
			{ value++; }
		}
	}
	return value;
...
int traverseNeighbors() {
	...
	for (int i = 0; i < size; ++i) {
		...
		if (y < y_current) { y_current = y; current = i; }
	...
	fromKtoIJ(current, &col1, &col2);  //获得最佳邻居与当前solution不同的两个列编号
	row1 = solution[col1];
	solution[col1] = solution[col2];
	solution[col2] = row1;
	return y_current;
